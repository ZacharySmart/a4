...
void MyTunes::executeCommand(Command cmd){
    ...
    else if (cmd.isCommand(CMD_UNFOLLOW)) executeCMDUNFOLLOW(cmd);
    else if (cmd.isCommand(CMD_SEARCH)) executeCMDSEARCH(cmd);
}
...
void MyTunes::executeDeleteUserPlaylist(Command cmd){
	...
	if(playlist == NULL) return;
    //detatch playlist from any other observing playlists to prevent expectation of follow by null playlist
    for (typename vector<User*>::iterator it = (users.getCollection()).begin() ; it != (users.getCollection()).end(); ++it){
        Playlist *matchList= (*it)->findPlaylist(playlistName);
        if ((matchList!=NULL)&(matchList!=playlist)){
            matchList->dettach(*playlist);
        }
    }
...
//Playlist Following Commands

void MyTunes::executeCMDFOLLOW(Command cmd){
    enum arguments{FOLLOW, _U, UID1, PNAME, _F, UID2};
    User* user1=users.findByUserID(cmd.getToken(UID1));
    if(user1 == NULL) return;
    Playlist *playlist1 = user1->findPlaylist(cmd.getToken(PNAME));
    if (playlist1==NULL) return;
    User* user2=users.findByUserID(cmd.getToken(UID2));
    if(user2 == NULL) return;
    Playlist * playlist2 = user2->findPlaylist(cmd.getToken(PNAME));
    if (playlist2==NULL) return;
    playlist2->attach(*playlist1);
    //playlist1->update(playlist2);
    view.printOutput("EXECUTING: FOLLOW PLAYLIST " + cmd.getCommandString());
}

void MyTunes::executeCMDUNFOLLOW(Command cmd){
    enum arguments {UNFOLLOW, _U, UID1, PNAME, _F, UID2};
    User* user1=users.findByUserID(cmd.getToken(UID1));
    if(user1 == NULL) return;
    Playlist *playlist1 = user1->findPlaylist(cmd.getToken(PNAME));
    if (playlist1==NULL) return;
    ...
    Playlist * playlist2 = user2->findPlaylist(cmd.getToken(PNAME));
    if (playlist2==NULL) return;
    playlist2->dettach(*playlist1);
    view.printOutput("EXECUTING: UNFOLLOW PLAYLIST " + cmd.getCommandString());
}

//Search command

void MyTunes::executeCMDSEARCH(Command cmd){
    bool foundMatch;
    bool prevmatch;
    string tempString=(cmd.getCommandString());
    std::string::size_type n = 0;
    n = tempString.find_first_not_of( " \t", n );
    n = tempString.find_first_of( " \t", n );
    tempString.erase( 0,  tempString.find_first_not_of( " \t", n ));//remove first word 'search'
    //printf ("after removing first word:%s\n", tempString.c_str());
    tempString=StrUtil::toTitleCase(tempString);
    tempString= StrUtil::stripPunctuation(tempString); //remove extra punctuation
    //printf ("after removing punctuation:%s\n", tempString.c_str());
    tempString= StrUtil::firstSpaceRemove(tempString); //remove extra whitespace at beginning
    //printf ("after removing beginning whitespace:%s\n", tempString.c_str());
    tempString= StrUtil::removeExtraWhitespace(tempString);//remove extra spaces in between words
    printf("Searching database for <%s>...\n ",tempString.c_str());
    
    //songs
    //printf ("reach\n");
    int count=0;
    vector<Song*>::iterator NOPASSs=(songs.getCollection()).end();
    for(vector<Song*>::iterator itr = (songs.getCollection()).begin();(itr != NOPASSs);itr++){
        //printf("song itr: %d \n", itr);
        if (*itr==NULL){
            continue;
        }
        ...
        if (song!=NULL){
        //Song asong= *song
        if ((StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(song->getTitle()),tempString)==0)
             ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(to_string(song->getID())),tempString)==0)
             ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(song->getComposer()),tempString)==0)){
            printf("%s\n",(song->toString()).c_str());
            foundMatch=true;
    ...
    //tracks
    count=0;
    vector<Track*>::iterator NOPASSt=(tracks.getCollection()).end();
    for(vector<Track*>::iterator itr = (tracks.getCollection()).begin();(itr != NOPASSt); itr++){
        //printf("track itr: %d \n", *itr);
        if (*itr==NULL){
            break;
        }
        ...
        if (track!=NULL){
        //Song asong= *song
        if ((StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(track->getMp3FileName()),tempString)==0)
            ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(to_string(track->getID())),tempString)==0)){
    ...
    //recordings
    count=0;
    vector<Recording*>::iterator NOPASSr=(recordings.getCollection()).end();
    for(vector<Recording*>::iterator itr = (recordings.getCollection()).begin();(itr != NOPASSr); itr++){
        //printf("recording itr: %d \n", *itr);
        if (*itr==NULL){
            break;
        }
        count+=1;
        ...
        if (recording!=NULL){
        //Song asong= *song
        if ((StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(recording->getTitle()),tempString)==0)
            ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation((recording->getYear())),tempString)==0)
            ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation((recording->getProducer())),tempString)==0)
            ^(StrUtil::compareIgnoreCase(StrUtil::stripPunctuation(to_string(recording->getID())),tempString)==0))
        {
